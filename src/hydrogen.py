...
class Hydrogen(CommonDatabase):
	...
	def save_histogram_hydrogen_all_res_molecule(self, separator_receptor, separator_filename_mode, sqlctx, df):
		table = self.client.get_keyspace() + "." + "hydrogen_all_res_histogram_molecule"
		hydrogen_all_res = sqlctx.sql("select ligand_atom, accept_or_donate, receptor_residue, receptor_atom, distance, angle, pose from hydrogen_all_resFILE")
		hydrogen_all_resRDD = hydrogen_all_res.rdd.map(lambda p: Row(molecule=get_molecule_name_from_filename_mode(separator_receptor, separator_filename_mode, str(p[6]) ), ligand_atom=str(p[0]), accept_or_donate=str(p[1]), receptor_residue=str(p[2]), receptor_atom=str(p[3]), distance=float(p[4]), angle=float(p[5]), pose=str(p[6]) ))
		hydrogen_all_res = sqlctx.createDataFrame(hydrogen_all_resRDD)
		hydrogen_all_res.createOrReplaceTempView("hydrogen_all_res_histogram_molecule")
		hydrogen_all_res = sqlctx.sql("select molecule, count(molecule) as number from hydrogen_all_res_histogram_molecule group by molecule")
		for row in hydrogen_all_res.collect():
			sql_query = """
				INSERT INTO @$$$$$$$$$$$$@
				(molecule, number)
